...
using ws_client::BytesCallback_Imp;
using ws_client::Callback_Imp;
...
class _DummyWebSocket : public ws_client::WebSocket
{
  public:
    void             poll(int) {}
...
class _RealWebSocket : public ws_client::WebSocket
{
  public:
    ...
    virtual void _dispatchBinary(BytesCallback_Imp &callable)
    {
        ...
        while (true)
        {
                     ...
                     ws.opcode == wsheader_type::CONTINUATION)
            {
                if (ws.mask)
                {
                    for (size_t iter = 0; iter != ws.N; ++iter)
                    {
                        rxbuf[iter + ws.header_size] ^= ws.masking_key[iter & 0x3];
                    }
                }
            ...
            else if (ws.opcode == wsheader_type::PING)
            {
                ...
                std::string s_data(rxbuf.begin() + ws.header_size,
                                   rxbuf.begin() + ws.header_size + (size_t)ws.N);
                sendData(wsheader_type::PONG, s_data.size(), s_data.begin(), s_data.end());
            }
    ...
    void close()
    {
        ...
        uint8_t closeFrame[6] = {
          0x88, 0x80, 0x00, 0x00, 0x00, 0x00}; // last 4 bytes are a masking key
        std::vector<uint8_t> header(closeFrame, closeFrame + 6);
        txbuf.insert(txbuf.end(), header.begin(), header.end());
    }
};

ws_client::WebSocket::pointer
from_url(const std::string &url, bool useMask, const std::string &origin)
{
    char host[512];
    int  port;
    char path[512];
    ...
    // fprintf(stderr, "ws_client: connecting: host=%s port=%d path=/%s\n", host, port, path);
    ...
    int flag = 1;
    setsockopt(
      sockfd, IPPROTO_TCP, TCP_NODELAY, (char *)&flag, sizeof(flag)); // Disable Nagle's algorithm
#ifdef _WIN32
    u_long on = 1;
    ...
    return ws_client::WebSocket::pointer(new _RealWebSocket(sockfd, useMask));
}
